...
run_hicpro() {
    ...
    if [[ -f "${OPTION_DIR}/DNaseC_config_hicpro_XX.txt" ]]; then
        cp "${OPTION_DIR}/DNaseC_config_hicpro_XX.txt" "$config_file"
    else
    ...
    # 定义 BAM 文件路径
    HICPRO_BAM="${hicpro_dir}/hicpro_results/bowtie_results/bwt2/${SAMPLE_NAME}_trimLk3/${SAMPLE_NAME}__hg38XX.bwt2pairs.bam"
...
run_rm() {
    ...
    rm -f "${OUTPUT_DIR}/05_hicpro/hicpro_results/hic_results/data/${SAMPLE_NAME}_trimLk3/${SAMPLE_NAME}__hg38XX.bwt2pairs.validPairs"
